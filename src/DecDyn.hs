...
data ExprDyn where
  ...
  DApp  :: ExprDyn -> ExprDyn -> ExprDyn
  DNow :: DeclarationDyn -> ExprDyn
  DAt :: DeclarationDyn -> (Int, Dynamic) -> ExprDyn
...
instance Show ExprDyn where
  show (DLeaf a) = "Leaf"
  show (DApp e1 e2) = "App (" ++ show e1 ++ ", " ++ show e2 ++ ")"
...
dec2Dyn dec = let
  ...
  thea = fromJust.(fromDynamic :: Dynamic -> Maybe a) in
...
dec2Dyn' themap dec@(Input id) = Map.insert id (DInp id, Map.singleton id (toDyn.(read :: String -> a)), Map.singleton id (toDyn.(fromJust.(parseMaybe parseJSON) :: Value -> a))) themap
...
exp2Dyn (App e1 e2) m = let
  ...
  in
  (m2, DApp (de1) (de2), mymergereaders r1 r2, mymergejsoners j1 j2)
...
exp2Dyn (dec :@ (i,d)) m = let
  m1 = dec2Dyn' m dec
  (ddec, readers, jsoners) = m1 Map.! getId dec in
  (m1,DAt (ddec) (i,toDyn d), readers, jsoners)
