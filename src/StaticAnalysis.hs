...
processExp (DApp e0 e1) fs = let
  (n0, fs0) = processExp e0 fs
  (n1, fs1) = processExp e1 fs0 in
  (n0++n1, fs1)
processExp (DNow decl) fs = ([(dgetId decl,[0])], addDecl fs decl)
processExp (DAt decl (off,_)) fs = ([(dgetId decl,[off])], addDecl fs decl)

addDecl :: FState -> DeclarationDyn -> FState
addDecl fs (DInp id) = St (Set.insert id (indexes fs)) (Map.insert id [] (neighbours fs))
addDecl fs (DOut (id,e)) | Set.member id (indexes fs) = fs -- I've already been processed
                         | otherwise = let
                            newfs = fs {indexes = Set.insert id $ indexes fs}
                            (neighs, newfs') = processExp e newfs in
                            newfs' { neighbours = Map.insert id neighs $ neighbours newfs'}
...
checkGraph :: FGraph -> ((Int,[Vert]), (Vert,Vert,Int))
...
checkGraph g = let
  ...
  cyclescheck = sequence_ $ map (foldl checkerr (Right Nothing)) vcycles
...
paths g forbidden a b = let
    ...
    tail = maybe [] (map (\s -> (s, [a,b]))) $ neighs Map.!? b
