...
data AttackData = AD { name :: String, filt :: Flow -> Bool, markers :: [Marker], maxEntropy :: Int }
instance Show AttackData where
  show (AD nm _ _ _) = nm

initer :: AttackData -> String -> Int -> (String -> Stream (Set.Set String))
                      -> [Map.Map String Value]
                      -> String
                      -> ([Map.Map String Data.Dynamic.Dynamic], Maybe (Set.Set String))
initer atk fileId flowcounter = traceIniter$argmaker
  where
  argmaker addr = ["/Users/felipe.gorostiaga/Documents/HLola/examples/retroactive/retriever.py", "--attack", name atk, "--until", show flowcounter, "--addr", show addr, "--file", fileId]
 
flowRetriever atk fileId = let
  jsons = unsafePerformIO (getAllJSONs (readproc "/Users/felipe.gorostiaga/Documents/HLola/examples/retroactive/retriever.py" ["--attack", name atk, "--file", fileId]))
  flows = map (\m -> (fromJust.parseMaybe parseJSON) (m Map.! "flow")) jsons
  in flows

threshold :: AttackData -> Int
threshold (AD _ _ (m:_) _) = thrshld m

data Marker = Marker { markerEnum :: MarkerEnum, thrshld :: Int }
instance Show Marker where
  show (Marker en _) = show en
  
data MarkerEnum = BPS | PPS deriving Show

data Flow = Flow { proto :: Protocol, dA :: String, sA :: String, dP :: String, sP :: String, fA :: Bool, fS :: Bool, fR :: Bool, fF :: Bool, fP :: Bool, fU :: Bool, fX :: Bool, pkts :: Int, bts :: Int, ts :: Int, te :: Int} deriving (Generic, Read, FromJSON)
